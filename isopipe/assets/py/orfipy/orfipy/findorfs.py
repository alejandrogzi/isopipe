...
from orfipy import orfipy_core as oc
import subprocess
...
def worker_map(arglist):
    ...
    for x in range(len(arglist[-2])):
        if arglist[-2][x]:
            ...
            file_streams += (
                open(
                    os.path.join(arglist[-1], arglist[2] + ".orfipytmp_" + str(x)), "w"
                ),
            )
        else:
            file_streams += (None,)
...
def start_multiprocs(
    ...
    for name, seq, *rest in seqs:
        ...
        if total_read_bytes + 1000000 >= _MEMLIMIT or process_long:
            print(
                "Processing {0:d} bytes".format(cummulative_read_bytes),
                end="\r",
                flush=True,
                file=sys.stderr,
            )
...
def main(
    ...
    ut.print_success(
        "Processed {0:d} sequences in {1:.2f} seconds".format(_total_seqs, duration)
    )
    logr.info(
        "Processed {0:d} sequences in {1:.2f} seconds".format(_total_seqs, duration)
    )
