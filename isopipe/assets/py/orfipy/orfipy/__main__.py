...
def validate_codons(starts, stops):
    ...
    for c in starts:
        if not len(c) == 3:
            ...
            return False
        if not (
            (c[0] in validalphabets)
            and (c[1] in validalphabets)
            and (c[2] in validalphabets)
        ):
            print("Invalid start codon:" + c)
            return False
    for c in stops:
...
def main():
    ver = orfipy.version.__version__
    parser = argparse.ArgumentParser(
        description="orfipy: extract Open Reading Frames (version {})".format(ver),
        usage="""
    ...
    parser.add_argument(
        "--procs", help="Num processor cores to use\nDefault:mp.cpu_count()"
    )
    parser.add_argument(
        "--single-mode",
        help="Run in single mode i.e. no parallel processing (SLOWER). If supplied with procs, this is ignored. \nDefault: False",
        dest="single",
        action="store_true",
        default=False,
    )
    ...
    parser.add_argument(
        "--table",
        help="The codon table number to use or path to .json file with codon table.\nUse --show-tables to see available tables compiled from: https://www.ncbi.nlm.nih.gov/Taxonomy/Utils/wprintgc.cgi?chapter=cgencodes\nDefault: 1",
        default="1",
    )
    parser.add_argument(
        "--start",
        help='Comma-separated list of start-codons. This will override start codons described in translation table. E.g. "--start ATG,ATT"\nDefault: Derived from the translation table selected',
    )
    parser.add_argument(
        "--stop",
        help='Comma-separated list of stop codons. This will override stop codons described in translation table. E.g. "--start TAG,TTT"\nDefault: Derived from the translation table selected',
    )

    # output options
    parser.add_argument(
        "--outdir", help="Path to outdir\ndefault: orfipy_<infasta>_out", action="store"
    )
    ...
    parser.add_argument(
        "--min",
        help="Minimum length of ORF, excluding stop codon (nucleotide)\nDefault: 30",
        default=30,
    )
    parser.add_argument(
        "--max",
        help="Maximum length of ORF, excluding stop codon (nucleotide)\nDefault: 1,000,000,000",
        default=1000000000,
    )
    parser.add_argument(
        "--strand",
        help="Strands to find ORFs [(f)orward,(r)everse,(b)oth]\nDefault: b",
        default="b",
        choices=["f", "r", "b"],
    )
    parser.add_argument(
        "--ignore-case",
        help="Ignore case and find ORFs in lower case sequences too. Useful for soft-masked sequences.\nDefault: False",
        default=False,
        dest="ignore_case",
        action="store_true",
    )
    ...
    parser.add_argument(
        "--partial-3",
        help='Output ORFs with a start codon but lacking an inframe stop codon. E.g. "ATG TTT AAA"\nDefault: False',
        default=False,
        dest="partial3",
        action="store_true",
    )
    parser.add_argument(
        "--partial-5",
        help='Output ORFs with an inframe stop codon lacking an inframe start codon. E.g. "TTT AAA TAG"\nDefault: False',
        default=False,
        dest="partial5",
        action="store_true",
    )
    ...
    parser.add_argument(
        "--include-stop",
        help="Include stop codon in the results, if a stop codon exists. This output format is compatible with TransDecoder's which includes stop codon coordinates\nDefault: False",
        default=False,
        dest="include_stop",
        action="store_true",
    )
    parser.add_argument(
        "--longest",
        help="Output a separate BED file for longest ORFs per sequence. Requires bed option.\nDefault: False",
        default=False,
        dest="longest",
        action="store_true",
    )
    parser.add_argument(
        "--by-frame",
        help='Output separate BED files for ORFs by frame. Can be combined with "--longest" to output longest ORFs in each frame. Requires bed option.\nDefault: False',
        default=False,
        dest="byframe",
        action="store_true",
    )
    ...
    parser.add_argument(
        "--chunk-size",
        help="""Max chunk size in MB. This is useful for limiting memory usage when processing large fasta files using multiple processes
    ...
    parser.add_argument(
        "infile",
        help="The input file, in plain Fasta/Fastq or gzipped format, containing Nucletide sequences",
        action="store",
        nargs="?",
    )
    ...
    ut.print_message(
        "Using translation table:", table["name"], "start:", starts, "stop:", stops
    )
    ...
    if args.longest or args.byframe:
        if not bed:
            print(
                "Please specify the bed output option if providing --longest or --byframe"
            )
            sys.exit(1)

    ###Estimate procs and chunk
    if not procs:
        procs = int(_max_procs * 0.7) + 1
