...
pub fn get_chr_from_path(path: &PathBuf) -> String {
    // INFO: {...}/step8_orf/seqs_free/chr10_KZ289072_fix:0/tmp_chunk_scaffold_10:0.index
    let basename = path
        .parent()
    ...
    basename.split(':').collect::<Vec<&str>>()[0].to_string() // INFO: chr10_KZ289072_fix
}
...
/// // let path3 = PathBuf::from("invalid_name.txt");
/// //
pub fn get_chr_from_filename(path: &PathBuf) -> String {
    let basename = path
        .file_name()
        .unwrap_or_else(|| panic!("ERROR: could not get basename from {:?}", path))
        .to_string_lossy();

    basename.split(':').collect::<Vec<&str>>()[0] // INFO: tmp_chunk_scaffold_10
        .split("chunk_")
        .last() // INFO: tmp_ - scaffold_10 -> scaffold_10
        .unwrap_or_else(|| panic!("ERROR: could not get chromosome from {:?}", path))
        .to_string()
}
