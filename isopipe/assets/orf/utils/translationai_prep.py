...
NF_FILE = (
    "/beegfs/projects/hillerlab/genome/src/ORFTree/nextflow/chunked_translationai.nf"
)
NF_CONFIG = (
    "/beegfs/projects/hillerlab/genome/src/ORFTree/nextflow/translation_ai_nxf.config"
)
...
def format_for_translationai(fasta_path: str, bed_path: str, output_path: str):
    ...
    with open(bed_path, "r") as bed_file:
        for line in bed_file:
            ...
            id_to_coords[cols[3]] = (
                f">{cols[0]}:{cols[1]}-{cols[2]}({cols[5]})({cols[3]})(0,0, )"
            )
            reformatted_ids.append(
                f">{cols[0]}:{cols[1]}-{cols[2]}({cols[5]})({cols[3]})(0,0, )"
            )
            ...
            for line in in_file:
                if not line.startswith(">"):
                    ...
                    seq_id = (
                        ...
                        .strip()
                        .split(" ")[0]
                        .replace("bosTau9_ncbiRefSeq_", "")
                    )
                    if id_to_coords.get(seq_id):
                        out_file.write(id_to_coords[seq_id] + "\n")
                    else:
                        logger.critical(
                            f"The sequence ID {seq_id} from {fasta_path} has no corresponding entry in {bed_path}. "
...
def trim_translationai_bed(input_bed, raw_translation_ai, output_path):
    ...
    with open(raw_translation_ai, "r") as results:
        for line in results:
            ...
            variant_dict[my_id] = [
                {
                    "orf_start": int(variants[0].split(",")[0]),
                    "orf_stop": int(variants[0].split(",")[1]) + 3,
                    "start_score": float(variants[0].split(",")[2]),
                    "stop_score": float(variants[0].split(",")[3]),
                }
            ]
            if len(variants) > 1:
                for variant in variants[1:]:
                    ...
                    old_list.append(
                        {
                            "orf_start": int(variant.split(",")[0]),
                            "orf_stop": int(variant.split(",")[1]),
    ...
    if dropped > 0:
        logger.warning(
            f"Dropped {dropped} variants out of {len(variant_dict.keys())} variants"
        )
    with open(output_path, "w") as out_file:
        out_file.writelines(final_rows)


def translationai_main(
    transcript_fasta: str, transcript_bed: str, tmp_dir, use_nextflow=False
):
    """
    ...
    subprocess.run(
        f"cat {transcript_fasta} | linearize_fasta > {tmp_dir}/TranslationAI/linear.fa",
        shell=True,
    )
    if use_nextflow:
        format_for_translationai(
            f"{tmp_dir}/TranslationAI/linear.fa",
            f"{transcript_bed}",
            f"{tmp_dir}/TranslationAI/linear.fa.formatted",
        )
        translation_ai_run = subprocess.run(
            f"cd {tmp_dir}/TranslationAI/ && nextflow {NF_FILE} -c {NF_CONFIG} --query linear.fa.formatted --out ./out.translationai -work-dir TEMP_TRANSLATIONAI",
            shell=True,
        )
    else:
        translation_ai_run = subprocess.run(
            f"cd {tmp_dir}/TranslationAI/ && run_translation_ai ../../{transcript_bed} ../../{transcript_fasta}.linear 1000 out.translationai",
            shell=True,
        )
    ...
    trim_translationai_bed(
        transcript_bed,
        f"{tmp_dir}/TranslationAI/out.translationai",
        f"{tmp_dir}/TranslationAI/translationai.trimmed.bed",
    )

    logger.info("Producing clean output file")
    clean_translationai_output(
        f"{tmp_dir}/TranslationAI/out.translationai",
