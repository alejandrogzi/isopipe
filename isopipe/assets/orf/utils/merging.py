...
def filter_by_relative_score_strict(group):
    ...
    :return:
    """
    sorted_group = group.sort_values(by='class1_probability', ascending=True).reset_index(drop=True)
    mask = (sorted_group['class1_probability'] >= (sorted_group['class1_probability'].iloc[-1] - 0.025))
...
def load_blast(input_fasta_path, results_path, exported_path) -> pd.DataFrame:
    ...
    for key in blast_data_dict.keys():
        ...
        prev_data['blast_percentage_aligned'] = blast_data_dict[key]['blast_alignment_len'] / query_lens[key]
        ...
        for line in blast_file:
            ...
            if blast_data_dict.get(my_id):
                ...
                if my_data['blast_id'].find("ORF") != -1:
                    ...
                    if strand == "+":
                        current_top_level_orf = (my_data['blast_id'], coding_start, coding_stop - coding_start)
                    elif strand == "-":
                        current_top_level_orf = (my_data['blast_id'], coding_stop, coding_stop - coding_start)
                    else:
                ...
                my_data['blast_coords'] = f"{chrom}|{coding_start}|{coding_stop}|{strand}"
                my_data['canonical_id'] = my_data['blast_id']
                my_data['genomic_coords'] = f"{chrom}|{coding_start}|{coding_stop}|{strand}"
                ...
                try:
                    ...
                    if strand == "+":
                        my_data['blast_nested_offset'] = abs(coding_start - current_top_level_orf[1]) / current_top_level_orf[2]
                        #print(my_id, abs(coding_start - current_top_level_orf[1]), current_top_level_orf[2])
                    else:
                        my_data['blast_nested_offset'] = abs(coding_stop - current_top_level_orf[1]) / current_top_level_orf[2]
    ...
    df = pd.DataFrame(data=blast_data_for_df, columns=["genomic_coords", "canonical_id", "blast_id", "blast_nested",
                                                       "blast_nested_offset", "blast_pid", "blast_e-value",
                                                       "blast_offset", "blast_percentage_aligned"])
...
def load_translation_ai(results_file, trimmed_bed) -> pd.DataFrame:
    ...
    with open(results_file, 'r') as translation_ai_data_file:
        for line in translation_ai_data_file:
            ...
            translation_ai_data_dict[my_id] = {"translationAI_orf_start": orf_start_score,
    ...
    with open(trimmed_bed, 'r') as translation_file:
        for line in translation_file:
            ...
            my_data['translationAI_coords'] = f"{chrom}|{coding_start}|{coding_stop}|{strand}"
            translation_ai_for_df.append(my_data)

    df = pd.DataFrame(data=translation_ai_for_df,
                           columns=["translationAI_coords", "translationAI_id", "translationAI_orf_start",
...
def load_toga(toga_results_file, query_annotation, overlap_file=None) -> pd.DataFrame:
    ...
    toga_data = toga_data.rename(columns={"pid": "toga_pid",
                                          "blosum": "toga_blosum",
                                          "label": "toga_label",
                                          "masked": "toga_masked"})
    toga_id_dict = dict()
    ...
    df = pd.DataFrame(data=toga_for_df, columns=["toga_coords", "toga_id", "toga_pid", "toga_blosum", "toga_label",
                                                 "toga_masked"])
    ...
    if overlap_file:
        ...
        overlap_df = pd.DataFrame(overlap_list, columns=['toga_id', 'canonical_id', 'toga_overlap_bp'])
        df = df.merge(overlap_df, left_on=['toga_id'], right_on=['toga_id'], how='left')

    return df

def load_toga_new(toga_results_file, query_annotation, overlap_file=None) -> pd.DataFrame:
    """
    Load TOGA data for the common dataframe.
    :param toga_results_file: Merged TOGA results file
    :param query_annotation: Query annotation BED file
    ...
    toga_data = pd.read_csv(toga_results_file, delimiter="\t", names=TOGA_LABELS, index_col=0)
    ...
    if overlap_file:
        ...
        with open(overlap_file, 'r', encoding='utf-8') as overlap_file:
            for line in overlap_file:
                ...
                overlap_list.append((a_id, id_components[0], id_components[1], bp_overlap))

        overlap_df = pd.DataFrame(overlap_list, columns=['toga_id', 'genomic_coords', 'canonical_id', 'toga_overlap_bp'])
        df = df.merge(overlap_df, left_on=['toga_id'], right_on=['toga_id'], how='left')

    return df


def load_toga_two(toga_results_file, query_annotation, toga_masked_file, overlap_file=None) -> pd.DataFrame:
    """
    Load TOGA data for the common dataframe.
    :param toga_results_file: Merged TOGA results file
    :param query_annotation: Query annotation BED file
    ...
    """
    TOGA_LABELS =["TOGA_ID", "label", "pid", "blosum"]
    toga_data = pd.read_csv(toga_results_file, delimiter="\t", names=TOGA_LABELS, usecols=[0,1,2,3], index_col=0)
...
def add_masked_labels(df, masked_path):
    ...
    mask_data.rename(columns={"projection":"TOGA_ID", "query_codon":"masked"}, inplace=True)
