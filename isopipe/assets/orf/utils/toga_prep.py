...
def export_df_orfs(df, input_bed, output_path):
    ...
    with open(output_path, "w") as f:
        for entry in df.itertuples():
            ...
            if orf_coords[-1] == "+":
                source_row.trim_to_genomic_position_upstream(
                    source_row.genomic_position_to_sequence_position(int(orf_coords[1]))
                )
                source_row.trim_to_genomic_position_downstream(
                    source_row.genomic_position_to_sequence_position(int(orf_coords[2]))
                )
            elif orf_coords[-1] == "-":
                source_row.trim_to_genomic_position_upstream(
                    source_row.genomic_position_to_sequence_position(int(orf_coords[2]))
...
def combine_toga_data(pid_blosum_data, label_data, masking_data, output_path):
    ...
    label_df = pd.read_csv(
        label_data, delimiter="\t", index_col=0, names=["projection", "label"]
    )
